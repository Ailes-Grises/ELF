...
void Elf::eh_parser(Device &bd){
	for(int i=0;i<EI_NIDENT;i++) ehdr.e_ident[i]=bd.get8bit(bd);

	ehdr.e_type=bd.get16bit(bd);
	ehdr.e_machine=bd.get16bit(bd);
	ehdr.e_version=bd.get32bit(bd);
	ehdr.e_entry=bd.get64bit(bd);
	ehdr.e_phoff=bd.get64bit(bd);
	ehdr.e_shoff=bd.get64bit(bd);
	ehdr.e_flags=bd.get32bit(bd);
	ehdr.e_ehsize=bd.get16bit(bd);
	ehdr.e_phentsize=bd.get16bit(bd);
	ehdr.e_phnum=bd.get16bit(bd);
	ehdr.e_shentsize=bd.get16bit(bd);
	ehdr.e_shnum=bd.get16bit(bd);
	ehdr.e_shstrndx=bd.get16bit(bd);
};
...
my $text_s=$text;
$text_s=~s/(get(8|16|32|64)bit|endl;)/@@@@@@@/g;
print $text_s;
